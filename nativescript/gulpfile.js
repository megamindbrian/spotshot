...
gulp.task('tns.Livesync', () => {
    return gulp.watch([`${SRC}**/*.tns.html`, `${SRC}/**/*.tns.scss`, `${SRC}/**/*.component.ts`], {ignore: "**/*.DS_Store"})
        .on('change', (file) => {
...
gulp.task('tns.Livesync.Phone', () => {
    return gulp.watch([`${SRC}**/*.tns.phone.html`, `${SRC}/**/*.tns.phone.scss`, `${SRC}/**/*.component.ts`], {ignore: "**/*.DS_Store"})
        .on('change', (file) => {
            var outputDest = file.replace(SRC, DEST);
            outputDest = outputDest.substring(0, outputDest.lastIndexOf('/'));
            gulp.src([file])
